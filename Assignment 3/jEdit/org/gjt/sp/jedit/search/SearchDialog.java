...
public class SearchDialog extends EnhancedDialog implements EBComponent
{
	...
	/**
	 * This this the box list to show the history search
	 */
	private JComboBox historyComboBox;
	
	/**
	 * THe panel that showing the history list 
	 * @return JPanel of searchedList
	 */
	public JPanel searchedList()
	{
		JPanel panel = new JPanel();
		JLabel label = new JLabel("Searched List: ");
		historyComboBox = new JComboBox();
		panel.add(label);
		panel.add(historyComboBox);
		return panel;
	}
	...
	private SearchDialog(View view)
	{
		...
		// add the search history panel to the dialog
		content.add(BorderLayout.AFTER_LAST_LINE,searchedList());
	...
	class ButtonActionHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent evt)
		{
			...
			{
				ok();
				//only in this situation, the search list will store the search string
				int i = historyComboBox.getItemCount();
				//size is not larger than 5
				if(i>=5)
				{
					String a = (String)historyComboBox.getItemAt(1);
					String b = (String)historyComboBox.getItemAt(2);
					String c = (String)historyComboBox.getItemAt(3);
					String d = (String)historyComboBox.getItemAt(4);
					
					//historyComboBox.repaint();
					historyComboBox.removeAllItems();
					historyComboBox.addItem(a);
					historyComboBox.addItem(b);
					historyComboBox.addItem(c);
					historyComboBox.addItem(d);
					// add the item from SearchAndReplace class
					historyComboBox.addItem(SearchAndReplace.getSearchString());
				}else {
					historyComboBox.addItem(SearchAndReplace.getSearchString());
				}
			}
